...
export async function POST(request: Request){
    ...
    const { villeDepart, villeArriver, covoitDate  } = await request.json();

    // Effectue un appel à une API externe avec les données reçues
    const apiExterne = await fetch(API_URL + API_ROUTES.rechercheCovoit, {
      method: "POST",// Méthode HTTP
      headers: {
        "Content-type": "application/json", // Spécifie que le corps est au format JSON
      },
      body: JSON.stringify({villeDepart, villeArriver, covoitDate }),
    });
