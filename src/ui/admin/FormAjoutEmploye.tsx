        ...
        <section className="my-5">
            ...
            <div className="container w-75 py-5">
                <form onSubmit={handleSubmit}>
                    <InputForm classDiv="mb-3" id="utilNom" type="text" value={utilNom} name="utilNom" label="Nom :" onChange={(value: string) => setUtilNom(value)} required={true} />
                    {utilNom && utilNom.length < 3 && (
                        <p className="text-danger">Le nom doit comporter au moins 3 caractères.</p>
                    )}
                    <InputForm classDiv="mb-3" id="utilPrenom" type="text" value={utilPrenom} name="utilPrenom" label="Prénom :" onChange={(value: string) => setUtilPrenom(value)} required={true} />
                    {utilPrenom && utilPrenom.length < 3 && (
                        <p className="text-danger">Le prénom doit comporter au moins 3 caractères.</p>
                    )}
                    <InputForm classDiv="mb-3" id="utilEmail" type="email" value={utilEmail} name="utilEmail" label="Email :" onChange={(value: string) => setUtilEmail(value)} required={true} />
                    {utilEmail && !validationEmail(utilEmail) && (
                        <p className="text-danger">Format invalide</p>
                    )}
                    <InputForm classDiv="mb-3" id="utilMdp" type="password" value={utilMdp} name="utilMdp" label="Mot de passe :" onChange={(value: string) => setUtilMdp(value)} required={true} />
                    {utilMdp && !validationMdp(utilMdp) && (
                        <p className="text-danger">Format invalide</p>
                    )}
                    <InputForm classDiv="mb-3" id="confirmeMdp" type="password" value={confirmeMdp} name="confirmeMdp" label="Confirmation du mot de passe:" onChange={(value: string) => setConfirmeMdp(value)} required={true} />
                    {confirmeMdp && !mdpIdentique(utilMdp, confirmeMdp) && (
                        <p className="text-danger">Le mot de passe ne correspond pas</p>
                    )}
                    <InputForm classDiv="mb-3" id="utilNaissance" type="date" value={utilNaissance} name="utilNaissance" label="Date de naissance :" onChange={(value: string) => setUtilNaissance(value)} required={true} />
