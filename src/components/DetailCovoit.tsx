...
import ParticiperCovoiturage from "./ParticiperCovoiturage";
type Props = {
    connecter: boolean;
}

export default function DetailCovoit({ connecter }: Props) {
    ...
    const savePath = () =>{
        ...
        localStorage.setItem("covoitPath", path);
    }
        ...
        <main className={`${styles.rechercheBackground}`}>
            ...
            <section className="bg-white shadow p-3">
                ...
                <div className="d-flex justify-content-center gap-3 my-5">
                    {connecter ? (
                        <ParticiperCovoiturage date={dataDetails.covoitDate} resaCovoit={dataDetails.covoitId}/>
                    ) : (
                        <Link href={"/connexion"} className="btn btn-info" onClick={savePath}>Pour participer, vous devez vous connecter.</Link>
                    )}
                    {dataDetails.nbCom > 0 && (
